...
def plimc(args=None, stdout=None):
    ...
    cli_parser.add_argument('-r', '--root', help="root for template")
    cli_parser.add_argument(
        '-e', '--encoding', default='utf-8', help="content encoding")
    ...
    sys.path.insert(0, '')
    preprocessor = EntryPoint.parse(
        'x={}'.format(preprocessor_path)).load(False)

    # Render to html, if requested
    # ----------------------------
    if args.html:
        root_dir = args.root or os.path.dirname(os.path.abspath(args.source))

        template_file = args.source
