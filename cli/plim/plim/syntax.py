...
class BaseSyntax(object):
    ...
    # Parsers
    # ----------------------------------
    PARSE_DOCTYPE_RE = re.compile(
        'doctype\s+(?P<type>[0-9a-z\.]+)', re.IGNORECASE)
    ...
    # This constant uses l.LITERAL_CONTENT_PREFIX and
    # l.LITERAL_CONTENT_SPACE_PREFIX
    ...
    PARSE_VARIABLE_RE = re.compile(
        "=(?P<prevent_escape>=)?(?P<explicit_space>,)?\s*(?P<line>.*)", re.IGNORECASE)
    PARSE_COMMENT_RE = re.compile('/.*')

    PARSE_STATEMENTS_RE = re.compile(
        '-\s*(?P<stmnt>if|for|while|with|try)(?P<expr>.*)')
    PARSE_FOREIGN_STATEMENTS_RE = re.compile(
        '-\s*(?P<stmnt>unless|until)(?P<expr>.*)')
    PARSE_PYTHON_NEW_RE = re.compile(
        '---[-]*(?P<excl>\!)?\s*(?P<expr>[^-].*)?')
    PARSE_PYTHON_CLASSIC_RE = re.compile(
        '-\s*(?P<python>py(?:thon)?(?P<excl>\!?))(?P<expr>\s+.*)?')
    PARSE_DEF_BLOCK_RE = re.compile('-\s*(?P<line>(?:def|block)(?:\s+.*)?)')
    PARSE_MAKO_ONE_LINERS_RE = re.compile(
    ...
    def __init__(self, custom_parsers=None):
        ...
        # replacing them with None (see Django syntax vs. Mako syntax
        # definitions below).
        standard_parsers = (  # Order matters
            ...
            (self.PARSE_EXPLICIT_LITERAL_RE,
             l.parse_explicit_literal_with_embedded_markup),
