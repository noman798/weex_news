...
HG_IGNORE = set()
os.chdir(join(_env.PREFIX, "plim"))
...
def plim_map(filename):
    ...
    with open(filename, 'r') as infile:
        ...
        html = infile.read()
        for i in extract_all('"', '.plim"', html) + extract_all("'",
                                                                ".plim'", html) + extract_all(' ', '.plim', html):
            if not i.startswith('/'):
...
def compile(filename, html=True, print_path=True):
    ...
    if '/_' not in '/' + basename:
        html_path = join(_env.PREFIX, 'we', basename + '.we')
        mkpath(dirname(html_path))
        cmd = 'plimc -H %s -o %s --root %s' % (
            basename + ".plim", html_path, PLIM_PATH)
        r = envoy.run(cmd)
        if r.std_out.strip():
            print(r.std_out)
        if r.std_err.strip():
...
def monitor_plim_init():
    ...
    for pos, (dirpath, dirnames, filenames) in enumerate(
            walk(abspath(PLIM_PATH)), 1):
        for f in filenames:
            print(f)
            if f.endswith('.plim'):
